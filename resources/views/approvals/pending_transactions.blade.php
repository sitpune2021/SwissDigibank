                ...
                <table class="w-full whitespace-nowrap select-all-table" id="transactionTable1">
                    ...
                    <tbody>
                        @forelse ($pending_transactions as $pending_transaction)
                            <form action="{{ route('pending-transaction.update', $pending_transaction->id) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                <tr class="even:bg-secondary/5 dark:even:bg-bg3">
                                    <td class="py-5 px-6">{{ $pending_transaction->accounts?->branch?->branch_name ?? '' }}
                                    </td>
                                    ...
                                    <td class="py-5 px-6">
                                        <a href="{{ $pending_transaction->accounts?->members?->id
                                            ...
                                            : '#' }}"
                                            class="text-primary underline hover:text-primary/80">{{ ($pending_transaction->accounts?->members?->member_no ??
                                                ($pending_transaction->accounts?->members?->id
                                                    ? str_pad($pending_transaction->accounts->members->id, 6, '0', STR_PAD_LEFT)
                                                    : '-')) .
                                                ' - ' .
                                                ($pending_transaction->accounts?->members?->member_info_first_name ?? 'N/A') }}</a>
                                    </td>
                                    <td class="py-5 px-6">{{ $pending_transaction?->accounts?->account_type ?? '' }}</td>
                                    <td class="py-5 px-6">
                                        <a href="{{ $pending_transaction?->accounts ? route('accounts.show', base64_encode($pending_transaction->accounts->id)) : '#' }}"
                                            class="text-primary underline hover:text-primary/80">
                                            {{ $pending_transaction?->accounts?->account_no ?? '' }}
                                        </a>
                                    </td>
                                    <td class="py-5 px-6">
                                        <a href="{{ $pending_transaction ? route('transaction.show', base64_encode($pending_transaction->id)) : '#' }}"
                                    ...
                                    <td class="py-5 px-6">
                                        @if (strtolower($pending_transaction->payment_mode) == 'online')
                                            <select name="bank_account_id" id="bank_account_id"
                                                ...
                                                <option value="Fincare Small Finance Bank">Fincare Small Finance Bank
                                                </option>
                                    ...
                                    <td class="py-2 px-6">
                                        @if (strtolower($pending_transaction->payment_mode) == 'cheque')
                                            <input type="text" name="cheque_clearing_date"
                                                ...
                                                class="form-control bg-white width-100" required="required"
                                                readonly="readonly">
                                        @else
                                            <span class="text-gray-400 italic"></span>
                                        @endif
                                    ...
                                    <td class="py-2 px-6">
                                        <select name="payment_status" id="payment_status-3"
