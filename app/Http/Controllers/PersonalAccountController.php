...
use App\Models\PersonalLoanForeClosure;
use App\Models\PersonalLoanOtherCharge;
use App\Models\PersoalLoanExtension;
use Carbon\Carbon;
...
class PersonalAccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($goldLoan as $loan) {
            ...
            // Sum of collected EMI from personal_loan_transactions table
            $collectedAmount = DB::table('personal_loan_transactions')
                ->where('loan_id', $loan->id)
                ->sum('amount_collected');

            // Sum of other charges from personal_loan_other_charges table
            ...
            $loan->close_date = DB::table('personal_loan_fore_closures')
                ->where('loan_id', $loan->id)
                ->value('created_at');
        ...
        return view('personal.account.index', compact('goldLoan'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $savedPaidDates = DB::table('personal_loan_emi_status')
            ->where('loan_id', $id)
        ...
        $foreclosureDeposit = DB::table('personal_loan_fore_closures')
            ->where('loan_id', $id)
            ->sum('net_amount_k');
        ...
        $goldLoan = PersonalLoanApplication::with(['member.branch', 'branch', 'scheme', 'coApplicant1', 'guarantor1', 'PersoalLoanTransaction'])->find($id);
        ...
        switch ($interestType) {
            ...
            case 'flat_advanced_interest':
                ...
                $lastCurrentDebt = PersoalLoanTransaction::where('loan_id', $goldLoan->id)
                    ->orderByDesc('id')
                    ->value('current_debt');
        ...
        // EMI Transactions
        $transactions = DB::table('personal_loan_transactions')
            ->where('loan_id', $id)
        ...
        // Foreclosure Deposit
        $foreclosurePayments = DB::table('personal_loan_fore_closures')
            ->where('loan_id', $id)
        ...
        $closures = DB::table('personal_loan_fore_closures')
            ->where('loan_id', $id)
            ->select(
                'created_at as date',
        ...
        // OTHER CHARGES DUE
        $otherChargesTotal = DB::table('personal_loan_other_charges')
            ->where('loan_id', $id)
            ->sum('amount');
        ...
        $hasOverdueEmi = DB::table('personal_loan_emi_status')
            ->where('loan_id', $id)
    ...
    {
        $account = PersonalLoanApplication::findOrFail($id);

        // 1. TRANSACTIONS
        $transactions = PersoalLoanTransaction::where('loan_id', $id)
            ->get()
            ->map(function ($t) {
                return (object)[
        ...
        $otherCharges = PersonalLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'paid')
            ->get()
            ->map(function ($o) {
                return (object)[
        ...
        $foreclosure = PersonalLoanForeClosure::where('loan_id', $id)
            ->get()
            ->map(function ($f) {
                return (object)[
        ...
        return view(
            'personal.account.view-buttons.view-transactions.view_transactions',
    ...
    public function loanextension($id)
    {
        ...
        return view('personal.account.loan-extension', compact('goldLoan', 'currentDebt', 'banks'));
    }
    ...
    public function storeLoanExtension(Request $request, $id)
    {
        ...
        try {
            ...
            $extension = PersoalLoanExtension::create($validated);

            Log::info('✔ Loan Extension Saved Successfully!', [
                'extension_id' => $extension->id,
            ]);

            return redirect()->route('personal.account.show')->with('success', 'Loan Extension Successfully Added!');
        } catch (\Throwable $e) {
    ...
    public function updateEmiStatus(Request $request)
    {
        ...
        $emi = PersoalLoanTransaction::where('loan_id', $request->loan_id)
            ->where('status', 'UNPAID')
            ->orderBy('id', 'asc')
            ->first();
    ...
    public function linksaving($id)
    {
        ...
        return view('personal.account.view-buttons.link-saving-acc.link-saving-acc', compact('goldLoan', 'currentDebt', 'banks', 'savingAccounts'));
    }
    ...
    public function storeSavingAccount(Request $request, $loanId)
    {
        ...
        $loan = PersonalLoanApplication::findOrFail($loanId);

        // Update selected saving account
        DB::table('accounts')
            ->where('id', $request->saving_account_id)
            ->update([
                'loan_type'   => 'personal loan', // this page का loan category
                'loan_number' => $loan->id, // loan_applications का id
            ]);

        return redirect()->route('personal.account.show', $loanId)
    ...
    public function removeAccount(Request $request, $id)
    {
        ...
        try {
            ...
            PersoalLoanTransaction::where('loan_id', $loan->id)->delete();

            // 4) delete related rows from gold_loan_other_charges
            PersonalLoanOtherCharge::where('loan_id', $loan->id)->delete();
    ...
    {
        return view('personal.account.view-buttons.audit-trail.audit-trail');
    }

    // Gold loan debit other charges list page functionality
    public function showDebitChargesList($id)
    {
        $goldLoan = PersonalLoanApplication::with(['member'])->findOrFail($id);
        return view('personal.account.view-buttons.debit-other-charges.other-charges-list', compact('goldLoan'));
    }

    public function DebitOtherCharges($id)
    {
        $goldLoan = PersonalLoanApplication::with(['member', 'scheme', 'PersoalLoanTransaction'])->findOrFail($id);
        return view('personal.account.view-buttons.debit-other-charges.debit-other-charges', compact('goldLoan'));
    }
    ...
    public function storeDebitOtherCharges(Request $request, $id)
    {
        try {
            ...
            $charge = PersonalLoanOtherCharge::create([
                'loan_id'   => $id,
            ...
            $loan = PersonalLoanApplication::with('PersoalLoanTransaction')->findOrFail($id);
            $transaction = $loan->PersoalLoanTransaction()->first();
            ...
            return redirect()->route('personal.debitChargesList.form', $id)->with('success', 'Other charge debited successfully.');
    ...
    {
        $goldLoan = PersonalLoanApplication::with(['member', 'scheme', 'PersoalLoanTransaction'])->findOrFail($id);
        $totalDue = PersonalLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'unpaid')
            ->sum('amount');
        $banks = Bank::all();
        return view('personal.account.view-buttons.debit-other-charges.clear-dues', compact('goldLoan', 'totalDue', 'banks'));
    }
    ...
    public function clearDue(Request $request, $id)
    {
        ...
        try {
            ...
            // Step 1: Create a record in personal_loan_other_charges
            $clearDue = PersonalLoanOtherCharge::create([
                'loan_id'          => $id,
            ...
            return redirect()->route('personal.debitChargesList.form', $id)->with('success', 'Due cleared successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function uploadDocuments($id)
    {
        ...
        return view('personal.account.documents.upload_documents', compact('loan'));
    }
    public function storeDocuments(Request $request, $id)
    {
        ...
        return redirect()->route('personal.account.show', $id)
