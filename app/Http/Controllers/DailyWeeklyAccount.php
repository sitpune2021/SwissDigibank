...
class DailyWeeklyAccount extends Controller
{
    ...
    public function mortgagePayEmi($id)
    {
        ...
        $goldLoan = DailyWeeklyApplication::with(['member', 'scheme'])
            ->findOrFail($id);
        ...
        $totalPaid = DB::table('daily_weekly_loan_transactions')
            ->where('loan_id', $id)
            ->where('status', 'paid')
            ->sum('amount_collected');

        // Next EMI (only unpaid ones)
        $nextEmi = DB::table('daily_weekly_loan_emi_status')
            ->where('loan_id', $id)
    ...
    private function calculateMortgagePayAmount($loanId)
    {
        ...
        $loan = DailyWeeklyApplication::find($loanId);

        $paid = DB::table('daily_weekly_loan_transactions')
            ->where('loan_id', $loanId)
            ->where('status', 'paid')
            ->sum('amount_collected');
    ...
    {
        Log::info('🟢 Daily Weekly EMI Store Started', [
            'loan_id' => $id,
            'payload' => $request->all()
        ]);
        ...
        try {
            ...
            Log::info('🟡 After Cleaning Amount', [
            ...
            $emi = DB::table('daily_weekly_loan_emi_status')
                ->where('loan_id', $id)
                ->whereIn('status', ['DUE', 'PARTIAL'])
                ->orderBy('emi_no')
                ->first();

            Log::info('🔎 EMI Found', ['emi' => $emi]);

            if (!$emi) {

                Log::info('🔥 FULL PAYMENT CASE');

                DB::table('daily_weekly_loan_transactions')->insert([
                    'loan_id' => $id,
                    'emi_no' => null,
                ...
                return redirect()
                    ->route('daily_weekly.account.show', $id)
                    ->with('success', 'Full Payment Submitted For Approval');
            }

            Log::info('💰 EMI PAYMENT CASE');
            ...
            Log::info('💾 EMI INSERTED SUCCESSFULLY');
    ...
    public function fourcloser($id)
    {
        ...
        try {
            ...
            $goldLoan = DailyWeeklyApplication::with(['member', 'branch', 'scheme'])
                ->findOrFail($id);
            ...
            // ================================
            $emiRemaining = DB::table('daily_weekly_loan_emi_status')
                ->where('loan_id', $id)
                ->sum('remaining_amount');

            $emiCount = DB::table('daily_weekly_loan_emi_status')
                ->where('loan_id', $id)
                ->count();
            ...
            return view(
                'daily_weekly.account.view-buttons.fore-close.fore-close',
    ...
    public function storeForeCloser(Request $request, $loanId)
    {
        ...
        try {
            ...
            // ✅ UPDATE LOAN STATUS
            // ===============================
            DailyWeeklyApplication::where('id', $loanId)
                ->update([
                    'status' => 4,
                    'updated_at' => now()
                ]);
