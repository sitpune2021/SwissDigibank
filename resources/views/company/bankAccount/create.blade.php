...
<div class="main-inner">
    ...
    <div class="col-span-12 box lg:col-span-12">
        <form method="POST"
            ...
            <div class="grid grid-cols-2 gap-4 mt-6 xl:mt-8 xxxxxl:gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="bank_id" {{ $isView ? 'disabled' : '' }}
                        ...
                        <option value="{{ $bank->id }}" {{ old('bank_id', $bankAccount->bank_id ?? '') == $bank->id ?
                            'selected' : '' }}>
                            {{ $bank->name }}
                        </option>
                        @endforeach
                ...
                <div class="col-span-2 md:col-span-1 mt-3">
                    <x-datepicker-disabled label="ACCOUNT OPEN DATE" name="account_open_date"
                        inputId="account_open_date"
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="account_type" {{ $isView ? 'disabled' : '' }}
                        ...
                        <option value="saving" {{ old('account_type', $bankAccount->account_type ?? '') == 'saving' ?
                            'selected' : '' }}>
                            Saving
                        </option>

                        <option value="current" {{ old('account_type', $bankAccount->account_type ?? '') == 'current' ?
                            'selected' : '' }}>
                            Current
                        </option>

                        <option value="overdraft" {{ old('account_type', $bankAccount->account_type ?? '') ==
                            'overdraft' ? 'selected' : '' }}>
                            Overdraft
                        </option>
                    </select>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <div class="flex gap-6">
                        <label class="flex gap-2">
                            <input type="radio" name="account_active" value="1" {{ old('account_active',
                                $bankAccount->account_active ?? '') == 1 ? 'checked' : '' }}
                            {{ $isView ? 'disabled' : '' }}>
                            <p>Yes</p>
                        </label>

                        <label class="flex gap-2">
                            <input type="radio" name="account_active" value="0" checked {{ old('account_active',
                                $bankAccount->account_active ?? '') == 0 ? 'checked' : '' }}
                            {{ $isView ? 'disabled' : '' }}>
                            <p>No</p>
                        </label>
                    </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <div class="flex gap-6">
                        <label class="flex gap-2">
                            <input type="radio" name="use_for_printing" value="1" {{ old('use_for_printing',
                                $bankAccount->use_for_printing ?? '') == 1 ? 'checked' : '' }}
                            {{ $isView ? 'disabled' : '' }}>
                            <p>Yes</p>
                        </label>

                        <label class="flex gap-2">
                            <input type="radio" name="use_for_printing" value="0" checked {{ old('use_for_printing',
                                $bankAccount->use_for_printing ?? '') == 0 ? 'checked' : '' }}
                            {{ $isView ? 'disabled' : '' }}>
                            <p>No</p>
                        </label>
                    </div>
            ...
            <div class="text-lg uppercase mt-6 text-center font-semibold">Link With Software Accounting</div>
            ...
            <div class="grid grid-cols-2 gap-4 mt-6 xl:mt-8 xxxxxl:gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="branch_id" {{ $isView ? 'disabled' : '' }}
                        ...
                        <option value="{{ $branch->id }}" {{ old('branch_id', $bankAccount->branch_id ?? '') ==
                            $branch->id ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                        @endforeach
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="accounting_bank" {{ $isView ? 'disabled' : '' }}
                        ...
                        <option value="">-- Select Bank --</option>
                        @foreach ($banks as $bank)
                        <option value="{{ $bank->id }}" {{ old('accounting_bank', $bankAccount->accounting_bank ?? '')
                            == $bank->id ? 'selected' : '' }}>
                            {{ $bank->name }}
                        </option>
                        @endforeach
