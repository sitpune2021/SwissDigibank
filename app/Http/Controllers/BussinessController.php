...
use App\Models\BussinessLoanScheme;
use App\Models\Member;
use App\Models\Branch;
use App\Models\Scheme;
use App\Models\BussinessLoanApplication;
use App\Models\Calculator;
use App\Models\BussinessLoanCreditscore;
use Carbon\Carbon;
...
class BussinessController extends Controller
{
    ...
    {       
        //$schemes = businessScheme::all();
        // paginate(10) => 10 records per page
        $schemes = BussinessLoanScheme::orderBy('id', 'desc')->paginate(10);
        return view("business.schemes.index", compact('schemes'));
    } 
  

    public function create()
    {
        return view("business.schemes.create");
    }
    ...
    public function store(Request $request)
    {
        ...
        BussinessLoanScheme::create($validated);

        return redirect()
            ->route('business.schemes.index')
            ->with('success', 'Scheme created successfully!');
    }

    public function show($id)
    {
        $scheme = BussinessLoanScheme::findOrFail($id);
        return view('business.schemes.show', compact('scheme'));
    }

    public function edit($id)
    {
        $scheme = BussinessLoanScheme::findOrFail($id);
        return view('business.schemes.create', compact('scheme'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('business.schemes.index')
    ...
    {
        $scheme = BussinessLoanScheme::findOrFail($id);
        return view("business.schemes.view", compact('scheme'));
    }

    public function calculator()
    {
        $scheme = BussinessLoanScheme::all();
        return view("business.calculator.index", compact('scheme'));
    }
    ...
    public function calculateResult(Request $request)
    {
        ...
        if ($isManual) {
            ...
            $scheme = BussinessLoanScheme::findOrFail($request->scheme_id);
        ...
        //  Return to view
        return view('business.calculator.result', [
            'scheme' => $scheme,
            'is_manual' => $isManual,
            'loan' => $loan,
    ...
    public function appindex()
    {
        ...
        $applications = BussinessLoanApplication::with(['creditScores'])
            ->latest()
            ->paginate(10); // 10 records

        return view("business.applications.index", compact('applications'));
    }
    ...
    public function appcreate() 
    {
        ...
        return view("business.applications.create", compact('members','branch','scheme','banks'));
    }
    ...
    public function storeLoanApplication(Request $request)
    {
        ...
        try {
        $loanApplication = BussinessLoanApplication::create($request->only([
            'application_date',
            'member_id',
            'co_applicant_1_id',
            'co_applicant_2_id',
    ...
    {
        $application = BussinessLoanApplication::with([
            'member',
            'coApplicant1',
            'guarantor1',
        ...
        return view("business.applications.view", compact('application'));
    }


   public function appedit($id)
    {
        $application = BussinessLoanApplication::with(['member', 'scheme'])->findOrFail($id);

        // Dropdown data 
        $members = Member::all();
        $schemes = BussinessLoanScheme::all();
        $branch = Branch::all();
        $scheme = BussinessLoanScheme::all();
        $banks = Bank::pluck('name', 'id'); // ['id' => 'name']

        return view('business.applications.create', compact('application', 'members', 'schemes','branch', 'scheme','banks'));
    }
    ...
    public function appupdate(Request $request, $id)
    {
        ...
        $application = BussinessLoanApplication::findOrFail($id);
    ...
    public function linepropertyindex()
    {
        ...
        $applications = BussinessLoanApplication::with(['creditScores', 'branch', 'member'])
            ->whereNotIn('status', [1, 0])
            ->latest()
            ->get(['id', 'status']);

        return view("business.lineproperty.index", compact('applications'));
    }
