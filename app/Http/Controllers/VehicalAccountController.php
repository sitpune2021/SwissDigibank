...
use App\Models\VehicalLoanForeClosure;
use App\Models\VehicalLoanOtherCharge;
use App\Models\VehicalLoanExtension;
use Carbon\Carbon;
...
class VehicalAccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($goldLoan as $loan) {
            ...
            // Sum of collected EMI from vehical_loan_transactions table
            $collectedAmount = DB::table('vehical_loan_transactions')
                ->where('loan_id', $loan->id)
                ->sum('amount_collected');

            // Sum of other charges from vehical_loan_other_charges table
            ...
            $loan->close_date = DB::table('vehical_loan_fore_closures')
                ->where('loan_id', $loan->id)
                ->value('created_at');
        ...
        return view('vehical.account.index', compact('goldLoan'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        switch ($interestType) {
            ...
            case 'flat_advanced_interest':
                ...
                $lastCurrentDebt = VehicalLoanTransaction::where('loan_id', $goldLoan->id)
                    ->orderByDesc('id')
                    ->value('current_debt');
    ...
    {
        $account = VehicalApplication::findOrFail($id);

        // 1. TRANSACTIONS
        $transactions = VehicalLoanTransaction::where('loan_id', $id)
            ->get()
            ->map(function ($t) {
                return (object)[
        ...
        $otherCharges = VehicalLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'paid')
            ->get()
            ->map(function ($o) {
                return (object)[
        ...
        $foreclosure = VehicalLoanForeClosure::where('loan_id', $id)
            ->get()
            ->map(function ($f) {
                return (object)[
        ...
        return view(
            'vehical.account.view-buttons.view-transactions.view_transactions',
    ...
    public function storeLoanExtension(Request $request, $id)
    {
        ...
        try {
            ...
            $extension = VehicalLoanExtension::create($validated);
    ...
    public function updateEmiStatus(Request $request)
    {
        ...
        $emi = VehicalLoanTransaction::where('loan_id', $request->loan_id)
            ->where('status', 'UNPAID')
            ->orderBy('id', 'asc')
            ->first();
    ...
    public function storeSavingAccount(Request $request, $loanId)
    {
        ...
        $loan = VehicalApplication::findOrFail($loanId);

        // Update selected saving account
        DB::table('accounts')
            ->where('id', $request->saving_account_id)
            ->update([
                'loan_type'   => 'vehical loan', // this page का loan category
    ...
    public function removeAccount(Request $request, $id)
    {
        ...
        try {
            ...
            VehicalLoanTransaction::where('loan_id', $loan->id)->delete();

            // 4) delete related rows from gold_loan_other_charges
            VehicalLoanOtherCharge::where('loan_id', $loan->id)->delete();
    ...
    {
        $goldLoan = VehicalApplication::with(['member'])->findOrFail($id);
        return view('vehical.account.view-buttons.debit-other-charges.other-charges-list', compact('goldLoan'));
    }

    public function DebitOtherCharges($id)
    {
        $goldLoan = VehicalApplication::with(['member', 'scheme', 'VehicalLoanTransaction'])->findOrFail($id);
        return view('vehical.account.view-buttons.debit-other-charges.debit-other-charges', compact('goldLoan'));
    }
    ...
    public function storeDebitOtherCharges(Request $request, $id)
    {
        try {
            ...
            $charge = VehicalLoanOtherCharge::create([
                'loan_id'   => $id,
            ...
            $loan = VehicalApplication::with('VehicalLoanTransaction')->findOrFail($id);
            $transaction = $loan->VehicalLoanTransaction()->first();
            ...
            return redirect()->route('vehical.debitChargesList.form', $id)->with('success', 'Other charge debited successfully.');
    ...
    {
        $goldLoan = VehicalApplication::with(['member', 'scheme', 'VehicalLoanTransaction'])->findOrFail($id);
        $totalDue = VehicalLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'unpaid')
            ->sum('amount');
        $banks = Bank::all();
        return view('vehical.account.view-buttons.debit-other-charges.clear-dues', compact('goldLoan', 'totalDue', 'banks'));
    }
    ...
    public function clearDue(Request $request, $id)
    {
        ...
        try {
            ...
            // Step 1: Create a record in vehical_loan_other_charges
            $clearDue = VehicalLoanOtherCharge::create([
                'loan_id'          => $id,
            ...
            return redirect()->route('vehical.debitChargesList.form', $id)->with('success', 'Due cleared successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function uploadDocuments($id)
    {
        ...
        return view('vehical.account.documents.upload_documents', compact('loan'));
    }
    public function storeDocuments(Request $request, $id)
    {
        ...
        return redirect()->route('vehical.account.show', $id)
            ->with('success', 'Documents uploaded successfully');
    }
    public function destroyDocument($id)
    {
        $document = VehicalDocument::findOrFail($id);
