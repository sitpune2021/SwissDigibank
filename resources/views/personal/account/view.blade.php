...
<div class="main-inner">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4 lg:mb-8">
        <div class="flex items-start flex-col gap-2">
            <h1 class="text-2xl uppercase font-semibold">PERSONAL LOAN - {{$goldLoan->id}} </h1>
        </div>
    </div>

    <div class="flex flex-wrap gap-3">
        <a href="{{route('personal.account.transaction',$goldLoan->id)}}" class="btn-secondary uppercase px-2 py-2 rounded-10 ">
            View Transaction
        </a>
        @if(
            ...
            <a href="{{route('personal.account.pay-emi',$goldLoan->id)}}" class="btn-primary uppercase px-2 py-2 rounded-10 ">
                Pay Emi
            </a>
            @if(strtolower($goldLoan->scheme->gold_loan_setting) == 'flat_advanced_interest')
                <a href="{{ route('personal.account.extension',$goldLoan->id) }}" class="btn-error uppercase px-2 py-2 rounded-10 ">
                    LOAN EXTENSION
                </a>
            @endif
        ...
        @if(
            ...
            <a href="{{ route('personal.account.pay', $goldLoan->id) }}"
            class="btn-primary uppercase px-2 py-2 rounded-10">
                Pay
            </a>
        @endif  

        <a href="{{ route('personal.account.fourcloser', $goldLoan->id) }}" class="btn-error uppercase px-2 py-2 rounded-10 ">
            Fore CloseLoan
        </a>
        
        <a href="{{ route('personal.account.linksaving', $goldLoan->id) }}" class="btn-primary uppercase px-2 py-2 rounded-10 ">
        ...
        <div class="relative inline-block text-left">
            ...
            <div id="debitCharges"
                ...
                <div class="py-1">
                    <a href="{{route('personal.debitChargesList.form',$goldLoan->id)}}"
                        class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        Other Charges List
                    </a>
                    <a href="{{route('personal.debitOtherCharges.form',$goldLoan->id)}}"
                        class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        Debit Other Charges
                    </a>
                    <a href="{{route('personal.clear-due.form',$goldLoan->id)}}"
                        class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        Clear Dues
                    </a>
                </div>
        ...
        <form id="remove-account-form-{{ $goldLoan->id }}" action="{{ route('personal.remove', $goldLoan->id) }}" method="POST" class="inline">
            @csrf
        ...
        <a href="{{ route('personal.account.audit-trail') }}" class="btn-primary uppercase  px-2 py-2 rounded-10 ">
            Show audit trail
        </a>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    processButtons.forEach(btn => {
        btn.addEventListener("click", function () {
            ...
            // ⭐ AJAX CALL
            fetch("{{ route('personal.emi.saveEmiStatus') }}", {
                method: "POST",
                headers: {
