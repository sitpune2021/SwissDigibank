...
@section('content')
    ...
    <div class="main-inner">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('bussiness.applications.view-buttons.show-emi-chart', $application->id) }}" target="_blank"
                class="btn-primary  uppercase text-sm  px-2 py-2 rounded-10 ">
                Show EMI Chart
            </a>
            {{-- Submit button only when Draft --}}
            @if ($application->status == 0)
                <form action="{{ route('business.submitForApproval', $application->id) }}" method="POST">
                    @csrf
            ...
            @if ($application->status != 0 && $application->status != 3 && $application->status != 2)
                <a href="#" class="btn-warning  uppercase text-sm  px-2 py-2 rounded-10 ">
                    DISBURSE SETTINGS
                </a>
            ...
            @if ($application->status != 0 && $application->status != 3)
                <div class="relative inline-block text-left">
                    <!-- Button -->
                    <button type="button"
                        class="btn-secondary px-2 py-2 uppercase text-sm  rounded-10 flex items-center gap-2"
                    ...
                    <div id="printDropdown"
                        ...
                        <div class="py-1">
                            ...
                            <a href="{{ route('business_loan.payout_chart_business_loan_application_view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> EMI SCHEDULE CHART
                            </a>
                            <a href="{{ route('business_loan.sanction_letter-view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> SANCTION LETTER
                            </a>
                            <a href="{{ route('business_loan.loanAgreement-view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> LOAN AGREEMENT
                            </a>
                            <a href="{{ route('business_loan.disburse_letter.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> DISBURSE LETTER
                            </a>
                            <a href="{{ route('business_loan.promissory.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> PROMISSORY NOTE
                            </a>
                            <a href="{{ route('business_loan.undertaking_letter.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        ...
                        <table class="min-w-full text-sm text-left">
                            <thead class="bg-gray-100 text-gray-700">
                                <tr class="bg-secondary/5">
                                    <th class="px-4 uppercase py-2 font-semibold">CIBIL Type</th>
                                    <th class="px-4 uppercase py-2 font-semibold">CIBIL Score</th>
                                    <th class="px-4 uppercase py-2 font-semibold">Report Date</th>
                                    <th class="px-4 uppercase py-2 font-semibold">View Report</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                        ...
                        <table class="w-full border-collapse rounded-lg overflow-hidden  bg-white dark:bg-bg3">
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                <tr class=" text-center">
                                    <td class="font-semibold px uppercase py-2" uppercase colspan="2">
                                        Per EMI Charges
                                    </td>
                                </tr>
                    ...
                    <div class="overflow-x-auto mt-5 " id="goldLoanAppInfo">
                        <table class="w-full border-collapse rounded-lg overflow-hidden  bg-white dark:bg-bg3">
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                <tr class="border-b">
                                    <td class="font-semibold uppercase px-4 py-2">Amount Requested</td>
