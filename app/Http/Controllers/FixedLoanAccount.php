...
class FixedLoanAccount extends Controller
{

    // index page
    public function index(Request $request)
    {
        $goldLoan = FixedLoanApplication::with(['member', 'branch', 'scheme', 'DailyWeeklyLoanTransaction'])
            ->where('status', [2])
            ->orderBy('id', 'desc')
            ->paginate(10);
        ...
        foreach ($goldLoan as $loan) {
            ...
            // Sum of collected EMI from fixed_loan_transactions table
            $collectedAmount = DB::table('fixed_loan_transactions')
                ->where('loan_id', $loan->id)
                ->sum('amount_collected');


            // Sum of other charges from fixed_loan_other_charges table
            $otherCharges = DB::table('fixed_loan_other_charges')
                ->where('loan_id', $loan->id)
                ->sum('amount');

            // Foreclosure remaining amount
            $remainingAmount = DB::table('fixed_loan_fore_closures')
                ->where('loan_id', $loan->id)
            ...
            $loan->close_date = DB::table('fixed_loan_fore_closures')
                ->where('loan_id', $loan->id)
                ->value('created_at');
        ...
        return view('fixed_loan.account.index', compact('goldLoan'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $savedStatuses = DB::table('fixed_loan_emi_status')
            ->where('loan_id', $id)
            ->pluck('status', 'emi_no')
            ->toArray();

        $savedPaidDates = DB::table('fixed_loan_emi_status')
            ->where('loan_id', $id)
            ->pluck('paid_date', 'emi_no')
            ->toArray();
        $loan = FixedLoanApplication::findOrFail($id);

        $passbooks = Passbook::where('account_type', 'Fixed Account')
            ->where('account_no', $loan->id)
            ->get();
        // Total Deposit
        $totalDeposit = DB::table('fixed_loan_transactions')
            ->where('loan_id', $id)
            ->sum('amount_collected');

        // NEW: Total Foreclosure Amount
        $foreclosureDeposit = DB::table('fixed_loan_fore_closures')
            ->where('loan_id', $id)
            ->sum('net_amount_k');
        ...
        $otherChargesDeposit = DB::table('fixed_loan_other_charges')
            ->where('loan_id', $id)
            ->where('status', 'paid')
            ->sum('amount');
        ...
        $totalPayable = DB::table('fixed_loan_transactions')
            ->where('loan_id', $id)
            ->orderByDesc('id')
            ->value('total_payable') ?? 0;


        $goldLoan = FixedLoanApplication::with(['member.branch', 'branch', 'scheme', 'coApplicant1', 'guarantor1', 'DailyWeeklyLoanTransaction'])->find($id);
        ...
        switch ($interestType) {
            ...
            case 'reducing_emi':
                ...
                if ($monthlyRate <= 0) {
                    $eirEmi = round($principal / $emiCount, 2);
                } else {
                ...
                for ($i = 0; $i < $emiCount; $i++) {
                    ...
                    if ($monthlyRate <= 0) {
                        $interest = 0;
                        $principalComponent = $eirEmi;
                    } else {
                        $interest = $balance * $monthlyRate;
                        $principalComponent = $eirEmi - $interest;
                    }
        ...
        if (in_array($interestType, ['flat_emi', 'reducing_emi'])) {
            ...
            if ($monthlyRate <= 0) {
                $emiAmountCalc = round($principal / $emiCount, 2);
            } else {
                $emiAmountCalc = $principal * ($monthlyRate * pow(1 + $monthlyRate, $emiCount))
                    / (pow(1 + $monthlyRate, $emiCount) - 1);
            }
        ...
        // EMI Transactions
        $transactions = DB::table('fixed_loan_transactions')
            ->where('loan_id', $id)
        ...
        // Foreclosure Deposit
        $foreclosurePayments = DB::table('fixed_loan_fore_closures')
            ->where('loan_id', $id)
        ...
        $closures = DB::table('fixed_loan_fore_closures')
            ->where('loan_id', $id)
            ->select(
                'created_at as date',
        ...
        // OTHER CHARGES DUE
        $otherChargesTotal = DB::table('fixed_loan_other_charges')
            ->where('loan_id', $id)
            ->sum('amount');
        ...
        return view('fixed_loan.account.view', compact(
            'goldLoan',
            'principal',
            'firstEmiDate',
            'emiSchedule',
    ...
    {
        $goldLoan = FixedLoanApplication::with([
            'member.branch',
            'branch',
            'scheme',
            'coApplicant1',
    ...
    public function mortgagepayEmiLoan(Request $request, $id)
    {
        ...
        try {
            ...
            $loan = FixedLoanApplication::with('scheme')->findOrFail($id);
    ...
    {
        $account = FixedLoanApplication::findOrFail($id);
    ...
    public function loanextension($id)
    {
        ...
        $transactionDeposit = DB::table('fixed_loan_transactions')
            ->where('loan_id', $id)
            ->sum('amount_collected');
    ...
    public function payEmi(Request $request)
    {
        ...
        try {
            ...
            $loan = FixedLoanApplication::find($request->loan_id);
    ...
    public function storeForeCloser(Request $request, $loanId)
    {
        ...
        try {
            ...
            FixedLoanApplication::where('id', $loanId)->update(['status' => 4]);
    ...
    public function storeSavingAccount(Request $request, $loanId)
    {
        ...
        $loan = FixedLoanApplication::findOrFail($loanId);
    ...
    {
        $goldLoan = FixedLoanApplication::with(['member'])->findOrFail($id);
    ...
    {
        $goldLoan = FixedLoanApplication::with(['member', 'scheme', 'DailyWeeklyLoanTransaction'])->findOrFail($id);
    ...
    public function storeDebitOtherCharges(Request $request, $id)
    {
        try {
            ...
            $loan = FixedLoanApplication::with('DailyWeeklyLoanTransaction')->findOrFail($id);
    ...
    public function clearDue(Request $request, $id)
    {
        ...
        try {
            ...
            // Step 1: Create a record in fixed_loan_other_charges
