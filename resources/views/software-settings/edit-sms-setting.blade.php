@extends('layout.main')

@section('content')

<style>
    input[type="checkbox"] {
        width: 28px;
        height: 28px;
        accent-color: green;
        /* For modern browsers */
    }

    /* Fallback for browsers without accent-color support */
    input[type="checkbox"]:checked {
        background-color: green;
        border: none;
    }

    input[type="radio"] {
        width: 24px;
        height: 24px;
        accent-color: green;
        /* Modern browser support */
    }
</style>

<div class="main-inner">

    <div class=" flex flex-wrap items-center  justify-between gap-4 lg:mb-8">
        <div class="flex items-start flex-col  gap-2">
            <div class="flex items-center gap-3">
                <h1 class="text-lg font-semibold uppercase">
                   Edit SMS Setting - VL013
                </h1>
            </div>
        </div>
    </div>


    <div class="grid grid-cols-2 md:grid-cols-3 gap-6  min-h-screen md-4">
        <div class="col-span-2 md:col-span-1  dark:bg-bg3 rounded-2xl ">

            <div class="box">

                <div class="overflow-x-auto">
                    <div class="mb-4">
                        <label for="" class="block font-medium mb-2">
                          Your Entity ID 
                            <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="" name=""
                            class="w-full border rounded-10 px-3 py-3 text-sm bg-secondary/5 dark:bg-bg3"
                            placeholder="Enter Your Entity ID ">

                    </div>
                    @error('branch_id')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>

                <div class="overflow-x-auto">
                    <div class="mb-4">
                        <label for="" class="block font-medium mb-2">
                            Your Sender ID
                            <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="" name=""
                            class="w-full border rounded-10 px-3 py-3 text-sm bg-secondary/5 dark:bg-bg3"
                            placeholder="Enter Your Sender ID ">

                    </div>
                    @error('branch_id')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="overflow-x-auto">
                    <div class="mb-4">
                        <label for="" class="block font-medium mb-2">
                            Your SMS Signature
                            <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="" name=""
                            class="w-full border rounded-10 px-3 py-3 text-sm bg-secondary/5 dark:bg-bg3"
                            placeholder="Enter Your SMS Signature ">

                    </div>
                    @error('branch_id')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="overflow-x-auto">
                    <div class="mb-4">
                        <label for="" class="block font-medium mb-2">
                            Your SMS Template ID 
                            <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="" name=""
                            class="w-full border rounded-10 px-3 py-3 text-sm bg-secondary/5 dark:bg-bg3"
                            placeholder=" Your SMS Template ID  ">

                    </div>
                    @error('branch_id')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            
                <div class="flex gap-5 justify-center mt-5">
                    <button class="btn-primary uppercase">Update sms Setting</button>
                    <a href="{{ route('software-settings.sms-list') }} " class="btn-outline uppercase">Back</a>
                </div>
            </div>
        </div>
        <div class="col-span-2 md:col-span-1  dark:bg-bg3 rounded-2xl ">

            <div class="box">

                <div class="overflow-x-auto">
                   
                    <div class="mt-3 overflow-x-auto">
                        <table class="w-full  rounded-md text-left">
                            <tbody class="divide-y divide-gray-200">
                                <tr class="border-b">
                                    <td class="px-4 py-2 font-semibold uppercase bg-gray-50 w-1/3">
                                       Name
                                    </td>
                                    <td class="px-4 py-2">
                                       Vehicle Loan EMI PAID (static)
                                    </td>
                                </tr>

                                <tr class="border-b">
                                    <td class="px-4 py-2 font-semibold uppercase bg-gray-50">
                                      Code
                                    </td>
                                    <td class="px-4 py-2">
                                       VL013(static)
                                    </td>
                                </tr>

                                <tr class="border-b">
                                    <td class="px-4 py-2 font-semibold uppercase bg-gray-50">
                                        Message
                                    </td>
                                    <td class="px-4 py-2">
                                       Dear Customer, EMI of Rs. @amount_1 towards Loan a/c no @account_number_1 is PAID. @sms_signature (static)
                                    </td>
                                </tr>

                                <tr class="border-b">
                                    <td class="px-4 py-2 font-semibold uppercase bg-gray-50">
                                        Message Template
                                     ( For Approval from your DLT Partner )
                                    </td>
                                    <td class="px-4 py-2">
                                     Dear Customer, EMI of Rs. {#var#} towards Loan a/c no {#var#} is PAID. HUBERP(static)
                                    </td>
                                </tr>

                                <tr class="border-b">
                                    <td class="px-4 py-2 font-semibold uppercase bg-gray-50">
                                       Active
                                    </td>
                                    <td class="px-4 py-2">
                                       <div class="flex items-center gap-1">
                                    <span
                                class="block w-28 rounded-[30px] border border-n30 bg-primary/20 py-2 text-center text-xs text-primary dark:border-n500 dark:bg-bg3 xxl:w-16">
                                Yes
                            </span>
                             <span
                                class="block w-28 rounded-[30px] border border-n30 bg-error/20 py-2 text-center text-xs text-error dark:border-n500 dark:bg-bg3 xxl:w-16">
                                No
                            </span>
                                </div>
                                    </td>
                                </tr>

                               
                            </tbody>
                        </table>
                    </div>
                    

                </div>

            </div>
        </div>
    </div>
</div>
</div>






@endsection