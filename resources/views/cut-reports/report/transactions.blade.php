...
@section('content')
    ...
    <div class="main-inner">
        ...
        <div class=" mb-5 box flex justify-center gap-2 flex-col md:flex-row lg:flex-row">
            <form method="GET" action="{{ route('report.transactions') }}">
                <div class="flex flex-col justify-end">
                    ...
                    <div class="w-full mt-4">
                        ...
                        <select name="type" class="w-full border rounded-10 px-3 py-3 text-sm bg-secondary/5">
                            ...
                            <option value="fixed_loan">fixed Loan(static)</option>
                            <option value="vehicle_loan">vehicle Loan</option>
                            <option value="ccod_loan">CC/OD Limit</option>
                             <option value="daily_weekly">Daily/Weekly Loan</option>
                            
                        </select>
                    </div>

                    <div class="w-full text-center mt-4">
                        <button class=" btn-primary rounded-10 text-sm">
                            <i class="las la-search"></i> LIST TRANSACTIONS
                        </button>
                    </div>
                    ...
                    <tbody>
                        @foreach($transactions as $txn)
                            ...
                            ($txn->created_at ? \Carbon\Carbon::parse($txn->created_at)->format('d-m-Y') : '-')
                                            ...
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{ $txn->accounts->branch->branch_name ??
                            $txn->fdAccount->branch->branch_name ??
                            $txn->misaccount->branch->branch_name ?? $txn->rdaccount->branch->branch_name ??
                            $txn->ddsAccount->branch->branch_name ??
                            $txn->goldApplication->branch->branch_name ??
                            $txn->mortgageloanApplication->branch->branch_name ??
                            $txn->loanAgainstApplication->branch->branch_name ??
                            $txn->businessloanApplication->branch->branch_name ??
                            $txn->persoalLoanApplication->branch->branch_name ??
                            $txn->vehicalapplication->branch->branch_name ??
                            $txn->ccOdloanApplication->branch->branch_name ??
                              $txn->dailyWeeklyapplication->branch->branch_name ??
                            '-' }}
                                                </div>
                                            </td>
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                @php
                                                    $ids = [
                                                        optional($txn->goldApplication)->id,
                                                        optional($txn->mortgageloanApplication)->id,
                                                        optional($txn->loanAgainstApplication)->id,
                                                        optional($txn->businessloanApplication)->id,
                                                        optional($txn->persoalLoanApplication)->id,
                                                        optional($txn->vehicalApplication)->id,
                                                        optional($txn->ccOdloanApplication)->id,
                                                         optional($txn->dailyWeeklyapplication)->id,
                                                    ];

                                                    $value = '-';
                                                    foreach ($ids as $id) {
                                                        if ($id) {
                                                            $value = str_pad($id, 10, '0', STR_PAD_LEFT);
                                                            break;
                                                        }
                                                    }
                                                @endphp
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{
                            $txn->accounts->account_no ?? $txn->fdAccount->fd_no ??
                            $txn->misaccount->mis_account_no ??
                            $txn->rdaccount->rd_no ??
                            $txn->ddsAccount->dd_no ??
                             $value 
                            ?? '-'
                             }}
                                            ...
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{ $txn->payment_mode ?? $txn->mode ?? $txn->pay_mode ?? $txn->fee_mode ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{ $txn->amount ?? $txn->total_payable ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{ $txn->remarks ?? $txn->remark ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="text-start !py-5 px-6 min-w-[100px] cursor-pointer">
                                                <div class="flex items-center gap-1  uppercase">
                                                    {{ $txn->approve_status ?? $txn->status ?? '-' }}
                                                </div>
                                            </td>
                                        </tr>
                        @endforeach
