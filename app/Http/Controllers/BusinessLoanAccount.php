...
use App\Models\BusinessLoanForeClosure;
use App\Models\BusinessLoanOtherCharge;
use App\Models\BusinessLoanExtension;
...
class BusinessLoanAccount extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($goldLoan as $loan) {
            ...
            // Sum of collected EMI from business_loan_transactions table
            $collectedAmount = DB::table('business_loan_transactions')
                ->where('loan_id', $loan->id)
                ->sum('amount_collected');

            // Sum of other charges from business_loan_other_charges table
            ...
            $loan->close_date = DB::table('business_loan_fore_closures')
                ->where('loan_id', $loan->id)
                ->value('created_at');
        ...
        return view('bussiness.account.index', compact('goldLoan'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $savedPaidDates = DB::table('business_loan_emi_status')
            ->where('loan_id', $id)
            ->pluck('paid_date', 'emi_no')
            ->toArray();

        // Total Deposit
        $totalDeposit = DB::table('business_loan_transactions')
            ->where('loan_id', $id)
            ->sum('amount_collected');

        // NEW: Total Foreclosure Amount
        $foreclosureDeposit = DB::table('business_loan_fore_closures')
            ->where('loan_id', $id)
            ->sum('net_amount_k');
        ...
        $otherChargesDeposit = DB::table('business_loan_other_charges')
            ->where('loan_id', $id)
            ->where('status', 'paid')
            ->sum('amount');
        ...
        $totalPayable = DB::table('business_loan_transactions')
            ->where('loan_id', $id)
            ->orderByDesc('id')
            ->value('total_payable') ?? 0;


        $goldLoan = BusinessLoanApplication::with(['member.branch', 'branch', 'scheme', 'coApplicant1', 'guarantor1', 'BusinessLoanTransaction'])->find($id);
        ...
        switch ($interestType) {
            ...
            case 'flat_advanced_interest':
                ...
                $lastCurrentDebt = BusinessLoanTransaction::where('loan_id', $goldLoan->id)
                    ->orderByDesc('id')
                    ->value('current_debt');
        ...
        $totalPaid = BusinessLoanTransaction::where('loan_id', $id)->sum('amount_collected');
        ...
        // EMI Transactions
        $transactions = DB::table('business_loan_transactions')
            ->where('loan_id', $id)
        ...
        // Foreclosure Deposit
        $foreclosurePayments = DB::table('business_loan_fore_closures')
            ->where('loan_id', $id)
        ...
        $closures = DB::table('business_loan_fore_closures')
            ->where('loan_id', $id)
            ->select(
                'created_at as date',
        ...
        // OTHER CHARGES DUE
        $otherChargesTotal = DB::table('business_loan_other_charges')
            ->where('loan_id', $id)
            ->sum('amount');
        ...
        if ($hasDueEmi) {
            $payRoute = route('bussiness.account.pay-emi', $goldLoan->id);
    ...
    public function mortgagePayEmi($id)
    {
        ...
        $goldLoan = BusinessLoanApplication::with(['member', 'scheme'])
            ->findOrFail($id);
        ...
        $totalPaid = DB::table('business_loan_transactions')
            ->where('loan_id', $id)
            ->where('status', 'paid')
            ->sum('amount_collected');

        // Next EMI (only unpaid ones)
        $nextEmi = DB::table('business_loan_emi_status')
            ->where('loan_id', $id)
        ...
        return view('bussiness.account.view-buttons.pay-emi.pay_emi', compact(
            'goldLoan',
            'emiAmount',
            'remainingAmount',
            'overdueInterest',
    ...
    private function calculateMortgagePayAmount($loanId)
    {
        ...
        $loan = BusinessLoanApplication::find($loanId);

        $paid = DB::table('business_loan_transactions')
            ->where('loan_id', $loanId)
            ->where('status', 'paid')
            ->sum('amount_collected');
    ...
    public function mortgagepayEmiLoan(Request $request, $id)
    {
        ...
        try {
            ...
            $emi = DB::table('business_loan_emi_status')
                ->where('loan_id', $id)
            ...
            if (!$emi) {
                ...
                // 🔥 FULL PAYMENT CASE
                DB::table('business_loan_transactions')->insert([
                    'loan_id' => $id,
                    'emi_no' => null,
                ...
                return redirect()
                    ->route('bussiness.account.show', $id)
    ...
    {
        $account = BusinessLoanApplication::findOrFail($id);

        // 1. TRANSACTIONS
        $transactions = BusinessLoanTransaction::where('loan_id', $id)
            ->get()
            ->map(function ($t) {
                return (object)[
        ...
        $otherCharges = BusinessLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'paid')
            ->get()
            ->map(function ($o) {
                return (object)[
        ...
        $foreclosure = BusinessLoanForeClosure::where('loan_id', $id)
            ->get()
            ->map(function ($f) {
                return (object)[
        ...
        return view(
            'bussiness.account.view-buttons.view-transactions.view_transactions',
    ...
    public function loanextension($id)
    {
        ...
        $transactionDeposit = DB::table('business_loan_transactions')
            ->where('loan_id', $id)
            ->sum('amount_collected');
        ...
        return view('bussiness.account.loan-extension', compact('goldLoan', 'currentDebt', 'banks'));
    }
    ...
    public function storeLoanExtension(Request $request, $id)
    {
        ...
        try {
            ...
            $extension = BusinessLoanExtension::create($validated);

            Log::info('✔ Loan Extension Saved Successfully!', [
                'extension_id' => $extension->id,
            ]);

            return redirect()->route('bussiness.account.show')->with('success', 'Loan Extension Successfully Added!');
        } catch (\Throwable $e) {
    ...
    {
        $goldLoan = BusinessLoanApplication::with([
            'member.branch',
            'BusinessLoanTransaction',
            'branch',
            'scheme',
            'coApplicant1',
            'guarantor1'
        ...
        if ($currentDebt <= 0) {
            return view('bussiness.account.view-buttons.pay.pay', [
                'goldLoan' => $goldLoan,
                'banks' => $banks,
                'currentDebt' => 0,
                'payableAmount' => 0,
        ...
        return view('bussiness.account.view-buttons.pay.pay', compact(
            'goldLoan',
            'banks',
            'currentDebt',
            'payableAmount'
    ...
    public function payEmi(Request $request)
    {
        ...
        try {
            ...
            $request->validate([
                'loan_id' => 'required|exists:bussiness_loan_applications,id',
                'transaction_date' => 'required',
            ...
            $emiNo = $isFullPayment ? null : (BusinessLoanTransaction::where('loan_id', $loan->id)->max('emi_no') + 1);

            $transactionDate = Carbon::createFromFormat('d-m-Y', $request->transaction_date)
                ->format('Y-m-d');

            BusinessLoanTransaction::create([
                'loan_id' => $loan->id,
            ...
            return redirect()
                ->route('bussiness.account.show', $loan->id)
    ...
    public function fourcloser($id)
    {
        ...
        try {
            ...
            $goldLoan = BusinessLoanApplication::with(['member', 'branch', 'scheme'])
                ->findOrFail($id);
            ...
            // ================================
            $emiRemaining = DB::table('business_loan_emi_status')
                ->where('loan_id', $id)
                ->sum('remaining_amount');

            $emiCount = DB::table('business_loan_emi_status')
                ->where('loan_id', $id)
                ->count();
            ...
            return view(
                'bussiness.account.view-buttons.fore-close.fore-close',
    ...
    public function storeForeCloser(Request $request, $loanId)
    {
        ...
        try {
            ...
            // ===============================
            $save = BusinessLoanForeClosure::create([
                'loan_id'               => $loanId,
                'remaining_amount'      => $request->remaining_amount,
            ...
            // ✅ UPDATE LOAN STATUS
            // ===============================
            BusinessLoanApplication::where('id', $loanId)
                ->update([
                    'status' => 4,
                    'updated_at' => now()
                ]);
            ...
            return redirect()
                ->route('bussiness.account.show', $loanId)
    ...
    public function linksaving($id)
    {
        ...
        return view('bussiness.account.view-buttons.link-saving-acc.link-saving-acc', compact('goldLoan', 'currentDebt', 'banks', 'savingAccounts'));
    }
    ...
    public function storeSavingAccount(Request $request, $loanId)
    {
        ...
        $loan = BusinessLoanApplication::findOrFail($loanId);
        ...
        return redirect()->route('bussiness.account.show', $loanId)
    ...
    public function removeAccount(Request $request, $id)
    {
        ...
        try {
            ...
            BusinessLoanTransaction::where('loan_id', $loan->id)->delete();

            // 4) delete related rows from gold_loan_other_charges
            BusinessLoanOtherCharge::where('loan_id', $loan->id)->delete();

            // 5) commit
            DB::commit();

            // optional logging
            Log::info('bussiness loan removed', [
                'loan_id' => $loan->id,
    ...
    {
        return view('bussiness.account.view-buttons.audit-trail.audit-trail');
    }

    // Gold loan debit other charges list page functionality
    public function showDebitChargesList($id)
    {
        $goldLoan = BusinessLoanApplication::with(['member'])->findOrFail($id);
        return view('bussiness.account.view-buttons.debit-other-charges.other-charges-list', compact('goldLoan'));
    }

    public function DebitOtherCharges($id)
    {
        $goldLoan = BusinessLoanApplication::with(['member', 'scheme', 'BusinessLoanTransaction'])->findOrFail($id);
        return view('bussiness.account.view-buttons.debit-other-charges.debit-other-charges', compact('goldLoan'));
    }
    ...
    public function storeDebitOtherCharges(Request $request, $id)
    {
        try {
            ...
            $charge = BusinessLoanOtherCharge::create([
                'loan_id'   => $id,
            ...
            $loan = BusinessLoanApplication::with('BusinessLoanTransaction')->findOrFail($id);
            $transaction = $loan->BusinessLoanTransaction()->first();
            ...
            return redirect()->route('bussiness.debitChargesList.form', $id)->with('success', 'Other charge debited successfully.');
    ...
    {
        $goldLoan = BusinessLoanApplication::with(['member', 'scheme', 'BusinessLoanTransaction'])->findOrFail($id);
        $totalDue = BusinessLoanOtherCharge::where('loan_id', $id)
            ->where('status', 'unpaid')
            ->sum('amount');
        $banks = Bank::pluck('name', 'id');
        $savingAccounts = Account::where('account_type', 'SAVING')->pluck('account_no');

        return view('bussiness.account.view-buttons.debit-other-charges.clear-dues', compact('goldLoan', 'totalDue', 'banks', 'savingAccounts'));
    }
    ...
    public function clearDue(Request $request, $id)
    {
        ...
        Log::info('🟢 Starting bussiness due clearance process', [
            'loan_id'       => $id,
        ...
        try {
            ...
            if (!$goldTxn) {
                Log::warning('⚠️ bussiness transaction record missing', [
                    'loan_id' => $id,
                    'user_id' => Auth::id(),
                ]);
            ...
            // Step 1: Create a record in business_loan_other_charges
            $clearDue = BusinessLoanOtherCharge::create([
                'loan_id'          => $id,
            ...
            Log::info('🧾 bussiness Clear Due entry created successfully', [
                'loan_id'        => $id,
            ...
            Log::info('💰 bussiness current debt updated', [
                'loan_id'        => $id,
            ...
            Log::info('✅ bussiness due cleared successfully', [
                'loan_id'         => $id,
            ...
            return redirect()->route('bussiness.debitChargesList.form', $id)->with('success', 'Due cleared successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function addComment($id)
    {
        ...
        return view(
            'bussiness.account.comments.addComments',
            compact('comments')
        )->with('loan_id', $id);
    }
    public function storeComment(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'loan_id' => 'required|exists:business_loan_disbursements,id',
        ]);

        try {
            BusinessLoanComment::create([
