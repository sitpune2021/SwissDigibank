...
@section('content')
    ...
    <div class="main-inner">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('mortgage.account.transaction', $goldLoan->id) }}"
                class="btn-secondary uppercase px-2 py-2 text-sm rounded-10 ">
                View Transaction
            </a>
            @if (strtolower($goldLoan->scheme->gold_loan_setting) != 'no_emi')
                <a href="{{ $payRoute }}" class="btn-primary uppercase px-2 py-2 text-sm rounded-10">
                    {{ $payButtonText }}
                </a>
                @if (strtolower($goldLoan->scheme->gold_loan_setting) == 'flat_advanced_interest')
                    <a href="{{ route('mortgage.account.extension', $goldLoan->id) }}"
            ...
            <a href="{{ route('mortgage.account.fourcloser', $goldLoan->id) }}"
                class="btn-error uppercase px-2 py-2 rounded-10 text-sm">
                Fore CloseLoan
            </a>

            <a href="{{ route('mortgage.account.linksaving', $goldLoan->id) }}"
                class="btn-primary uppercase px-2 py-2 rounded-10 text-sm ">
            ...
            <form id="remove-account-form-{{ $goldLoan->id }}" action="{{ route('mortgage.remove', $goldLoan->id) }}"
            ...
            <a href="{{ route('mortgage.account.audit-trail') }}"
            ...
            <div class=" w-full overflow-x-auto   overflow-hidden">
                    ...
                    <div
                        ...
                        <div class=" flex gap-3">
                            <a href="javascript:void(0)" onclick="openModal()"
                                class="p-2 uppercase rounded-10 btn-primary">
                ...
                <div class="bg-white dark:bg-bg3 box shadow-md mt-5 rounded-10 overflow-hidden">
                    ...
                    <div class="flex items-center justify-between rounded-10 bg-secondary/5 text-black px-4 py-3 cursor-pointer"
                        ...
                        <a href="{{ route('mortgage.uploadDocuments', $loan->id) }}"
                                        ...
                                        @foreach ($documents as $doc)
                                            <tr class="border-b text-center">
                                                ...
                                                <td class="px-4 text-start py-2">
                                                    ...
                                                    <form action="{{ route('mortgage.documents.destroy', $doc->id) }}"
                                ...
                                <tbody>
                                    @foreach ($emiSchedule as $emi)
                                        ...
                                        <tr class="border-b {{ $emi['status'] == 'PAID' ? 'bg-green-50' : '' }}">
                                            ...
                                            <td class="p-2 border">
                                                @if ($emi['status'] === 'PAID')
                                                    <a href="{{ route('mortgage_loan.emi_receipt.view', [$goldLoan->id, $emi['emi_no']]) }}"
