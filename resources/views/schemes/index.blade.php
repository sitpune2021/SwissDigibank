        ...
        @if ($schemes->lastPage() > 1)
            <div class="flex col-span-12 gap-4 sm:justify-between justify-center items-center flex-wrap mt-4">
                <ul class="flex gap-2 md:gap-3 flex-wrap md:font-semibold items-center">
                    ...
                    @else
                        <li><a href="{{ $schemes->previousPageUrl() }}" class="hover:bg-primary text-primary border w-8 h-8 flex items-center justify-center rounded-full"><i class="las la-angle-left text-lg"></i></a></li>
                    @endif

                    {{-- Page Links --}}
                    @for ($i = 1; $i <= $schemes->lastPage(); $i++)
                        @if ($i == $schemes->currentPage())
                            <li><button class="bg-primary text-white border w-8 h-8 flex items-center justify-center rounded-full">{{ $i }}</button></li>
                        @else
                            <li><a href="{{ $schemes->url($i) }}" class="hover:bg-primary text-primary border w-8 h-8 flex items-center justify-center rounded-full">{{ $i }}</a></li>
                        @endif
                    @endfor

                    {{-- Next Page Link --}}
                    @if ($schemes->hasMorePages())
                        <li><a href="{{ $schemes->nextPageUrl() }}" class="hover:bg-primary text-primary border w-8 h-8 flex items-center justify-center rounded-full"><i class="las la-angle-right text-lg"></i></a></li>
                    @else
