    ...
    <div class="col-span-12 box lg:col-span-12">
        <form method="POST"
            ...
            <div class="col-span-2 md:col-span-1">
                ...
                <select name="penal_charge"
                    class="w-full text-sm bg-secondary/5 dark:bg-bg3 border border-n30 dark:border-n500 rounded-10 px-3 md:px-6 py-2 md:py-3">

                    <option value="">0.0</option>

                    @foreach([0.5,1,1.5,2,3,4,5] as $rate)
                    <option value="{{ $rate }}"
                        {{ old('penal_charge', $fdScheme->penal_charge ?? '') == $rate ? 'selected' : '' }}>
                        {{ $rate }}%
                    </option>
                    @endforeach
                </select>
                                ...
                                @for ($i = 0; $i < $rowCount; $i++)
                                    ...
                                    <td class="border border-gray-300 p-1">
                                        <select class="w-full border border-gray-300 rounded p-1"
                                            ...
                                            @foreach(\App\Models\FdSchemeSlab::PAYOUT_TYPES as $type)
                                            <option value="{{ $type }}"
                                                {{ old('rows.' . $i . '.payout_type', $slab->payout_type ?? '') == $type ? 'selected' : '' }}>
                                                {{ $type }}
                                            </option>
                                            @endforeach
