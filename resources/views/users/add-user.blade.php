            ...
            <form id="companyForm"
                    ...
                    <label for="rate" class="md:text-lg font-medium block mb-4">Employee
                        <select name="employee" id="employeeDropdown"
                            ...
                            @foreach ($employees as $emp)
                                <option value="{{ $emp->id }}"
                                    {{ old('employee', isset($user) && $user->emp_id == $emp->id ? 'selected' : '') }}>
                                    {{ $emp->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="text" name="designation" id="designation"
                        ...
                        class="w-full text-sm bg-secondary/5 dark:bg-bg3 border border-n30 dark:border-n500 rounded-10 px-3 md:px-6 py-2 md:py-3"
                        placeholder="Enter Designation like 'Executive'" @if ($isView) disabled @endif>
                    @error('designation')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="text" name="mobile_no" id="mobile_no"
                        value="{{ old('mobile_no', $user->mobile ?? '') }}" placeholder="Enter Mobile No."
                        class="w-full text-sm bg-secondary/5 dark:bg-bg3 border border-n30 dark:border-n500 rounded-10 px-3 md:px-6 py-2 md:py-3"
                        @if ($isView) disabled @endif>
                    @error('mobile_no')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
            ...
            @if ($isView) disabled @endif>
                    ...
                    <select name="permission_role" id="roleDropdown"
                        ...
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}"
                                {{ old('permission_role', isset($user) && $user->role_id == $role->id ? 'selected' : '') }}>
                                {{ $role->name }}</option>
                        @endforeach
                    </select>
                    ...
                    <select name="branch" id="branchDropdown"
                        ...
                        @foreach ($branches as $branch)
                            <option value="{{ $branch->id }}"
                                {{ old('branch', isset($user) && $user->branch_id == $branch->id ? 'selected' : '') }}>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="login_on_holidays" id="login_on_holidays"
                        ...
                        <option value="1"
                            {{ old('login_on_holidays', $user->login_on_holidays ?? '') == '1' ? 'selected' : '' }}> YES
                        </option>
                        <option value="0"
                            {{ old('login_on_holidays', $user->login_on_holidays ?? '') == '0' ? 'selected' : '' }}> NO
                        </option>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="user_active" id="user_active"
                        ...
                        <option value="1"
                            {{ old('user_active', $user->user_active ?? '') == '1' ? 'selected' : '' }}> YES</option>
                        <option value="0"
                ...
                <div class="col-span-2 md:col-span-1">
                    <label for="searchable_account" class="md:text-lg font-medium block mb-4">Searchable Accounts<span
                            class="text-red-500">*</span></label>
                    <label><input type="radio" name="searchable_account" value="1"
                            {{ old('searchable_account', '0') == '1' ? 'checked' : '' }}
                            @if ($isView) disabled @endif> Yes - All</label>
                    <label><input type="radio" name="searchable_account" value="0"
                            {{ old('searchable_account', '0') == '0' ? 'checked' : '' }}
                            @if ($isView) disabled @endif> No - Only Assigned</label>
