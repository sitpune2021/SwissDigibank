...
@push('script')
    <script>
        function calculateFD() {
            ...
            const payoutType = document.getElementById("interest_payout_type")
            .value; // e.g. QUARTERLY or CUMULATIVE_QUARTERLY
            ...
            function daysInclusive(a, b) {
                return Math.floor((dateISO(b) - dateISO(a)) / DAYMS) + 1;
            }
            ...
            function freqInfo(pt) {
                ...
                switch (pt) {
                    case "MONTHLY":
                        return {
                            perYear: 12, cumulative: false
                        };
                    case "QUARTERLY":
                        return {
                            perYear: 4, cumulative: false
                        };
                    case "HALF_YEARLY":
                        return {
                            perYear: 2, cumulative: false
                        };
                    case "YEARLY":
                        return {
                            perYear: 1, cumulative: false
                        };

                    case "CUMULATIVE_MONTHLY":
                        return {
                            perYear: 12, cumulative: true
                        };
                    case "CUMULATIVE_QUARTERLY":
                        return {
                            perYear: 4, cumulative: true
                        };
                    case "CUMULATIVE_HALF_YEARLY":
                        return {
                            perYear: 2, cumulative: true
                        };
                    case "CUMULATIVE_YEARLY":
                        return {
                            perYear: 1, cumulative: true
                        };
                    default:
                        return {
                            perYear: 4, cumulative: false
                        }; // safe default = QUARTERLY
                }
            }
    ...
    `;
            ...
            while (periodStart < maturityDate) {
                ...
                while (segStart <= periodEnd) {
                    ...
                    const willShowDueBy = isLastSegmentOfPeriod && (!cumulative || (addDays(segEnd, 1).getTime() ===
                        maturityDueBy.getTime()));
