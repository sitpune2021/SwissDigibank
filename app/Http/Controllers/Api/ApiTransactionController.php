   ...
   public function transactionHistory(Request $request)
{
    ...
    $items = collect($transactions instanceof \Illuminate\Pagination\LengthAwarePaginator ? $transactions->items() : $transactions);

    // Map transactions with correct running balance
    $items = $items->map(function ($t) use (&$currentBalance) {
        $balanceAtTransaction = $currentBalance;

        // Reverse calculation: subtract credit, add debit
        ...
        return [
            ...
            'balance' => number_format($balanceAtTransaction, 2),
        ];
    });
