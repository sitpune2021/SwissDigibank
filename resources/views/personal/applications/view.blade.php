    ...
    <div class="main-inner">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            @if ($application->status == 0)
                <form action="{{ route('personal.submitForApproval', $application->id) }}" method="POST">
                    @csrf
                ...
                <div class="relative inline-block text-left">
                    ...
                    <div id="printDropdown"
                        ...
                        <div class="py-1">
                            ...
                            <a href="{{ route('personal_loan.payout_chart_personal_loan_application_view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> EMI SCHEDULE CHART
                            </a>
                            <a href="{{ route('personal_loan.sanction_letter-view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> SANCTION LETTER
                            </a>
                            <a href="{{ route('personal_loan.loanAgreement-view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> LOAN AGREEMENT
                            </a>
                            <a href="{{ route('personal_loan.disburse_letter.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> DISBURSE LETTER
                            </a>
                            <a href="{{ route('personal_loan.promissory.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                <i class="las la-print text-secondary"></i> PROMISSORY NOTE
                            </a>
                            <a href="{{ route('personal_loan.undertaking_letter.view', $application->id) }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
