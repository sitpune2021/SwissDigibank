...
<div class="main-inner">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4 lg:mb-8">
        <div class="flex items-start flex-col gap-2">
            <h1 class="text-lg font-semibold">Transaction - RD{{ $transaction->id }}</h1>
            <!-- <p class="text-gray-500">
                <a href="#" class="text-gray-500">Recurring Deposits</a> >
                <a href="{{ route('rd-accounts.show', $rdAccount->id) }}" class="text-gray-500">
                    {{ $rdAccount->id }}
                </a> >
                <a href="{{ route('view.viewTransaction', $rdAccount->id) }}" class="text-gray-500">
                    Transactions
                </a> >
        ...
        <div class=" overflow-x-auto border  rounded-lg dark:bg-bg3 bg-white shadow-md p-4 ">
            <table class="w-full whitespace-nowrap text-sm text-left ">
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="border-t">
                        <td class="font-semibold px-4 py-2">RD/DD No.</td>
                        <td class="px-4 py-2">{{ $rdAccount->id ?? 'N/A' }}</td>
                    </tr>
                    <tr class="border-t">
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">
                            {{ $transaction->t_date ? \Carbon\Carbon::parse($transaction->t_date)->format('d M Y') : '-' }}
                        </td>
                    </tr>
                    <tr class="border-t">
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">
                            ...
                            <span
                                class="block w-28 rounded-[30px] border border-n30 bg-warning/10 py-2 text-center text-xs text-warning">
                                PENDING
                            </span>
                            @else
                            <span
                                class="block w-28 rounded-[30px] border border-n30 bg-error/10 py-2 text-center text-xs text-error">
                                REJECTED
                            </span>
                            @endif
                        </td>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">{{ ucfirst($transaction->payment_mode ?? '-') }}</td>
                    </tr>
                    <tr class="border-t">
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">{{ $rdAccount->branch->branch_name ?? 'N/A' }}</td>
                    </tr>
                    <tr class="border-t">
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">{{$transaction->paid_on ? \Carbon\Carbon::parse($transaction->t_date)->format('d M Y') : '-' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="box bg-white dark:bg-bg3 border shadow-md rounded-lg mt-4">
        <!-- Header -->
        <div class="flex justify-between items-center px-4 py-2 bg-secondary text-white rounded-t-lg">
            <h3 class="text-black font-semibold text-lg">RD ACCOUNT TRANSACTION AUDIT TRAIL</h3>
