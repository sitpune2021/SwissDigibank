    ...
    <div class="main-inner">
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-4 lg:mb-8">
            <div class="flex items-start flex-col gap-2">
                <h1 class="text-2xl uppercase font-semibold">VEHICAL LOAN - {{ $goldLoan->id }} </h1>
            </div>
        </div>

        <div class="flex flex-wrap gap-3">
            <a href="{{ route('vehical.account.transaction', $goldLoan->id) }}"
                class="btn-secondary uppercase px-2 py-2 rounded-10 ">
                View Transaction
            </a>
            @if (strtolower($goldLoan->scheme->gold_loan_setting) != 'no_emi')
                ...
                @if (strtolower($goldLoan->scheme->gold_loan_setting) == 'flat_advanced_interest')
                    <a href="{{ route('vehical.account.extension', $goldLoan->id) }}"
            ...
            @if (strtolower($goldLoan->scheme->gold_loan_setting) != 'flat_advanced_interest' &&
                ...
                <a href="{{ route('vehical.account.pay', $goldLoan->id) }}"
                    class="btn-primary uppercase px-2 py-2 rounded-10">
                    Pay
                </a>
            @endif

            <a href="{{ route('vehical.account.fourcloser', $goldLoan->id) }}"
                class="btn-error uppercase px-2 py-2 rounded-10 ">
                Fore CloseLoan
            </a>

            <a href="{{ route('vehical.account.linksaving', $goldLoan->id) }}"
                class="btn-primary uppercase px-2 py-2 rounded-10 ">
            ...
            <div class="relative inline-block text-left">
                ...
                <div id="debitCharges"
                    ...
                    <div class="py-1">
                        <a href="{{ route('vehical.debitChargesList.form', $goldLoan->id) }}"
                            class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            Other Charges List
                        </a>
                        <a href="{{ route('vehical.debitOtherCharges.form', $goldLoan->id) }}"
                            class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            Debit Other Charges
                        </a>
                        <a href="{{ route('vehical.clear-due.form', $goldLoan->id) }}"
                            class="flex items-center uppercase gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            Clear Dues
                        </a>
                    </div>
            ...
            <form id="remove-account-form-{{ $goldLoan->id }}" action="{{ route('vehical.remove', $goldLoan->id) }}"
            ...
            <a href="{{ route('vehical.account.audit-trail') }}" class="btn-primary uppercase  px-2 py-2 rounded-10 ">
                Show audit trail
            </a>
            ...
            <div class=" w-full overflow-x-auto   overflow-hidden">
                ...
                <div class="bg-white dark:bg-bg3 box shadow-md mt-5 rounded-10 overflow-hidden">
                    ...
                    <div class="flex items-center justify-between rounded-10 bg-secondary/5 text-black px-4 py-3 cursor-pointer"
                        ...
                        <a href="{{ route('vehical.uploadDocuments', $loan->id) }}"
                                        ...
                                        @foreach ($documents as $doc)
                                            <tr class="border-b text-center">
                                                ...
                                                <td class="px-4 text-start py-2">
                                                    ...
                                                    <form action="{{ route('vehical.documents.destroy', $doc->id) }}"
                ...
                bg-secondary/5 rounded-10 text-lg cursor-pointer"
                    ...
                    <div x-show="open" x-transition class="p-4 bg-white dark:bg-bg3">
                        ...
                        <div class="w-full flex items-center justify-center gap-4 mt-4">
                            ...
                            <!-- Add Comment -->
                            <a href="{{ route('vehical.addComment', $goldLoan->id) }}"
                            ...
                            @if ($comments->isNotEmpty())
                                <a class="btn-primary rounded-10 py-2"
                                    href="{{ route('vehical.addComment', $goldLoan->id) }}">
                                    VIEW ALL
                                </a>
                            @endif
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            processButtons.forEach(btn => {
                btn.addEventListener("click", function() {
                    ...
                    fetch("{{ route('vehical.emi.saveEmiStatus') }}", {
                            method: "POST",
                            headers: {
