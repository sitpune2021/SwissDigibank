    ...
    <div class="main-inner dark:bg-gray-900 dark:text-gray-200">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 w-full bg-white dark:bg-bg3 rounded-xl shadow p-6">
                <form class="space-y-6" action="{{ route('fd.account.updateInfo', $fdAccountDetail->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="mt-4">
                        ...
                        <select name="scheme_id"
                            ...
                            {{-- Selected Scheme --}}
                            @if ($fdAccountDetail->fdscheme)
                                <option value="{{ $fdAccountDetail->fdscheme->id }}" selected>
                                    {{ $fdAccountDetail->fdscheme->scheme_name }}
                                    ({{ $fdAccountDetail->fdscheme->tenure_year }}Y
                                    {{ $fdAccountDetail->fdscheme->tenure_month }}M)
                                </option>
                            @endif

                            {{-- Other Schemes --}}
                            @foreach ($schemes as $scheme)
                                @if ($fdAccountDetail->scheme_id != $scheme->id)
                                    <option value="{{ $scheme->id }}">
                                        {{ $scheme->scheme_name }}
                                        ({{ $scheme->tenure_year }}Y {{ $scheme->tenure_month }}M)
                                    </option>
                                @endif
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <select name="member_id" class="mt-2 px-3 py-3 bg-secondary/5 dark:bg-bg-3 w-full rounded-10 border">
                            {{-- First show selected member --}}
                            @if ($fdAccountDetail->member)
                                <option value="{{ $fdAccountDetail->member->id }}" selected>
                                    {{ $fdAccountDetail->member->id }} -
                                    {{ $fdAccountDetail->member->member_info_first_name }}
                                    {{ $fdAccountDetail->member->member_info_last_name }}
                                </option>
                            @endif
                            ...
                            @foreach ($members as $member)
                                @if ($fdAccountDetail->member_id != $member->id)
                                    <option value="{{ $member->id }}">
                                        {{ $member->id }} - {{ $member->member_info_first_name }}
                    ...
                    <div class="mt-4">
                        ...
                        <select name="minor_id" class="mt-2 px-3 py-3 bg-secondary/5 dark:bg-bg-3 w-full rounded-10 border">
                            <option value="">Select Minor</option>

                            {{-- Show current selected minor first --}}
                            @if ($fdAccountDetail->minor)
                                <option value="{{ $fdAccountDetail->minor->id }}" selected>
                                    {{ $fdAccountDetail->minor->first_name }} {{ $fdAccountDetail->minor->last_name }}
                                </option>
                            @endif

                            {{-- Other minors --}}
                            @foreach ($minors as $minor)
                                @if ($fdAccountDetail->minor_id != $minor->id)
                    ...
                    <div class="mt-4">
                        ...
                        <input  name="open_date" type="text" id="date" placeholder="DD/MM/YYYY"
                            value="{{ $fdAccountDetail->open_date ? \Carbon\Carbon::parse($fdAccountDetail->open_date)->format('d-m-Y') : '' }}"
                            class="mt-2 px-3 py-3 bg-secondary/5 dark:bg-bg-3 w-full rounded-10 border" />
                    </div>
                    ...
                    <div class="flex flex-col mt-6 sm:flex-row gap-3 justify-center">
                        ...
                        <a href="{{ route('fd-mis-schemes.fd_show', $fdAccountDetail->id) }}"
            ...
            <div class="w-full lg:w-1/3 box dark:bg-bg3 rounded-xl shadow overflow-hidden">
                <div class="bg-secondary/5 text-black px-4 py-3 flex justify-between  rounded-10 items-center">
                    <h4 class="font-semibold uppercase">Account Info</h4>
                </div>
                <div class="p-4">
                    <table class="w-full text-sm">
                        <tbody>
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="py-2">
                                    <a href="#" class="text-green-600 hover:underline">
                                        {{ $fdAccountDetail->member->id . ' ' . $fdAccountDetail->member->member_info_first_name . ' ' . $fdAccountDetail->member->member_info_last_name }}
                                    </a>
                                </td>
                            </tr>
                            <tr class="border-b dark:border-gray-700">
                            ...
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="py-2">
                                    {{ $fdAccountDetail->open_date ? \Carbon\Carbon::parse($fdAccountDetail->open_date)->format('d-m-Y') : '' }}
                                </td>
                            </tr>
                            <tr class="border-b dark:border-gray-700">
                                <td class="py-2 font-semibold dark:text-gray-300 uppercase">Status</td>
                                <td class="py-2"> {{ $fdAccountDetail->active == 1 ? 'Active' : 'Inactive' }}
                                </td>
                            </tr>
                            <tr>
                                <td class="py-2 font-semibold dark:text-gray-300 uppercase">Available Balance</td>
                                <td class="py-2"> {{ $fdAccountDetail->final_amount }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
