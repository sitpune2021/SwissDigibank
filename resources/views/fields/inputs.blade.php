        ...
        @foreach ($field['options'] as $optionValue => $optionLabel)
            ...
            <div class="flex items-center relative gap-2">
                <input id="{{ $radioId }}" 
                    ...
                    name="{{ $name }}"
                    value="{{ $optionValue }}"
                    class="h-6 w-6 accent-green-600 
                           {{ $isReadOnly ? 'pointer-events-none opacity-100' : '' }}"
                    {{ $value == $optionValue ? 'checked' : '' }}
                    {{ $isReadOnly ? 'disabled' : '' }}>

                <label for="{{ $radioId }}"
                    class="select-none text-sm md:text-base flex gap-2 items-center ml-2
...
@else
   @if ($name === 'incorporation_date')
        ...
        <input type="text" id="{{ $id }}" name="{{ $name }}"
            class="w-full text-sm bg-secondary/5 dark:bg-bg3 border border-n30 dark:border-n500 rounded-10 px-3 md:px-6 py-2 md:py-3"
            placeholder="Enter {{ strtolower($label) }}" value="{{ $formattedDate }}"
            {{ !empty($readonly) ? 'readonly' : '' }} />
