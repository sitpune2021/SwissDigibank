...
<div class="flex items-center gap-4">
    <label class="flex items-center gap-2">
        <input class="ms-2" type="radio" name="nominee" value="yes"
            ...
            {{ isset($rdAccount) && $rdAccount->nominees && $rdAccount->nominees->count() > 0 ? 'checked' : '' }}>
        Yes
    </label>
    <label class="flex items-center gap-2">
...
<div id="addMoreContainer"
    class="mt-2"
    @if(!isset($rdAccount) || !$rdAccount->nominees || $rdAccount->nominees->count() == 0) style="display:none;" @endif>
...
<div id="nomineeContainer"
    ...
    {{-- If nominees exist, pre-fill them --}}
    @if(isset($rdAccount) && $rdAccount->nominees && $rdAccount->nominees->count() > 0)
        @foreach($rdAccount->nominees as $index => $nominee)
            ...
            <div class="nominee-row flex flex-wrap justify-start gap-6">
                ...
                <div class="flex-center flex-1 min-w-[300px] max-w-full">
                    ...
                    <select name="nominees[{{ $index }}][relation]"
                        ...
                        @foreach(['Father','Mother','Spouse','Son','Daughter','Brother','Sister',
                        'Grandfather','Grandmother','Uncle','Aunt','Cousin','Nephew','Niece',
                        'Father-in-law','Mother-in-law','Brother-in-law','Sister-in-law',
                        'Son-in-law','Daughter-in-law','Guardian','Friend','Other'] as $relation)
                        <option value="{{ $relation }}" {{ $nominee->nominee_relation == $relation ? 'selected' : '' }}>
                            {{ $relation }}
                        </option>
                        @endforeach
                    </select>
        ...
        newNominee.innerHTML = `
            <div class="nominee-row flex flex-wrap justify-start gap-6">
                <div class="flex-center flex-1 min-w-[300px] max-w-full">
                    ...
                    <select name="nominees[${index}][relation]" class="w-full text-sm bg-secondary/5 dark:bg-bg3 border border-n30 dark:border-n500 rounded-10 px-3 md:px-6 py-2 md:py-3">
                        <option value="">Select Relation</option>
